		...
		if (cmd.getName().equals("qc") && args.length > 0) {
			if (args[0].equals("addp")) {
				...
				if (target == null) {
					...
					if (!yaml.contains("commands")) {
						...
						yaml.save();
						sender.sendMessage(
								ChatColor.GOLD + "Added " + args[1] + " to the queue with the command /" + command);
						return false;
					}
			...
			if (args[0].equals("message")) {
				...
				if (yaml.contains("messages")) {
					...
					yaml.save();
					sender.sendMessage(ChatColor.GOLD + "Added message to " + args[1] + "s queue!");
					return false;
				}
				if (!yaml.contains("messages")) {
					String oldArgs = args[1];
			...
			if (args[0].equals("addc")) {
				...
				if (target.isOnline()) {
					...
					sender.sendMessage(ChatColor.GREEN+"[QueueCommands] : "+target+" is now online, Successful run command.");
				} else {
					if (yaml.contains("consoleCommands")) {
						...
						yaml.save();
						sender.sendMessage(ChatColor.RED+"[QueueCommands] : "+target+" is now offline.");
						sender.sendMessage(ChatColor.RED + "Added " + args[1]
								+ " to the queue with the console command /" + consoleCmd);
						return false;
					}
					if (!yaml.contains("consoleCommands")) {
						String oldArgs = args[1];
